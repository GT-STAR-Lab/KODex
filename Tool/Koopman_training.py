...
def main(env_name, demo_file, num_demo, koopmanoption, velocity, save_matrix, matrix_file, config, control, error_type, visualize, unseen_test, rl_policy, folder_name, matrix_list, separate):
    ...
    if len(folder_name) == 0: # we do not explicitly define the folder name for saving the models
        folder_name = '/Hammer/'  # define the folder name for training
        unit_train = True
    ...
    # Load the trained matrix/model and roll it out in simulation only
        ...
        fig_hand_joints_unseen_demo = [''.join(fig_path_tmp[:-1]) + 'Joint_unseen(2530)/joint_' + str(i) + '.png' for i in range(num_hand)]
        if koopmanoption == "Drafted":
            ...
            if not Unseen_test:
                errors = koopman_error_visualization(env_name, Koopman, cont_koopman_operator, Training_data, Velocity, num_hand, num_obj, koopmanoption, error_type, Visualize)
                if error_type == 'demo':
                ...
                if not controller or error_type == 'demo':
                    ...
                    obj_ori_error_simu, obj_ori_error_simu_koopman, Computed_joint_unseen, success_rate = koopman_policy_control_unseenTest(env_name, Controller, Koopman, cont_koopman_operator, Training_data, Velocity, num_hand, num_obj, koopmanoption, Visualize)      
    ...
    if koopmanoption == "Drafted":
        ...
        for k in tqdm(range(len(Training_data))):
            ...
            obj_OriState = np.append(np.append(Training_data[k][0]['objpos'], np.append(Training_data[k][0]['objorient'], Training_data[k][0]['objvel'])), Training_data[k][0]['nail_goal'])
            ...
            for t in range(len(Training_data[k]) - 1):
                ...
                obj_OriState = np.append(np.append(Training_data[k][t + 1]['objpos'], np.append(Training_data[k][t + 1]['objorient'], Training_data[k][t + 1]['objvel'])), Training_data[k][t + 1]['nail_goal'])
...
def demo_playback(env_name, demo_paths, num_demo):
    ...
    Selected_timesteps = 31  # reduced horizon
