...
def main(env_name, demo_file, num_demo, koopmanoption, velocity, save_matrix, matrix_file, config, control, error_type, visualize, unseen_test, rl_policy, folder_name, matrix_list, separate, seed, first_demo):
    ...
    if len(folder_name) == 0: # we do not explicitly define the folder name
        folder_name = '/Hammer/NDP_cloning/'  # define the folder name for training
        unit_train = True
    ...
    # Load the trained matrix/model and roll it out in simulation only
        ...
        separate = 1
        if int(separate) == 2:
            lfd_net = LfDModel(network_size='large', policy_name="ngf", task_name='hammer')
        elif int(separate) == 1:
            lfd_net = LfDModel(network_size='median', policy_name="ngf", task_name='hammer')
        else:
            lfd_net = LfDModel(network_size='small', policy_name="ngf", task_name='hammer')
        ...
        if koopmanoption == "Drafted":
            ...
            if not Unseen_test:
                ...
                if not controller or error_type == 'demo':
                    ...
                    obj_ori_error_simu, obj_ori_error_simu_koopman, Computed_joint_unseen, success_rate = NGF_policy_control_unseenTest(env_name, Controller, lfd_net, Training_data, Velocity, num_hand, num_obj, koopmanoption, Visualize)      
