...
def main(env_name, demo_file, num_demo, koopmanoption, velocity, save_matrix, matrix_file, config, control, error_type, visualize, unseen_test, rl_policy, folder_name, matrix_list, separate, seed, first_demo):
    ...
    if len(folder_name) == 0: # we do not explicitly define the folder name
        folder_name = '/Hammer/Behaviour_cloning/parameter_tuning/'  # define the folder name for training
        unit_train = True
        ...
        if koopmanoption == "Drafted":
            ...
            if not Unseen_test:
                ...
                if not controller or error_type == 'demo':
                    ...
                    obj_ori_error_simu, obj_ori_error_simu_koopman, Computed_joint_unseen, success_rate = BC_policy_control_unseenTest(env_name, Controller, BC_agent, Training_data, Velocity, num_hand, num_obj, koopmanoption, Visualize)      
