...
import time
import shutil
from ngf.gym_wrapper.lfd_model import LfDModel
...
def main(env_name, demo_file, num_demo, koopmanoption, velocity, save_matrix, matrix_file, config, control, error_type, visualize, unseen_test, rl_policy, folder_name, matrix_list, separate, seed, first_demo):
    ...
    Controller_loc = '/home/yhan389/Desktop/KoopmanManipulation_DataSplit/demo_data/Relocation_selfPlay/demo_controller_heavy_hand/NN_controller_best.pt'
    multiple_test = True
    if env_name is "":
        print("Unknown env.")
        return
    ...
    if len(folder_name) == 0: # we do not explicitly define the folder name
        folder_name = '/Relocate/NDP_cloning/'  # define the folder name for training
        unit_train = True
    ...
    # Load the trained matrix/model and roll it out in simulation only
        ...
        separate = 2
        if int(separate) == 2:
            lfd_net = LfDModel(network_size='large', policy_name="ngf", task_name='reloc')
        elif int(separate) == 1:
            lfd_net = LfDModel(network_size='median', policy_name="ngf", task_name='reloc')
        else:
            lfd_net = LfDModel(network_size='small', policy_name="ngf", task_name='reloc')
        lfd_net.load_networks(model_dir=matrix_file, num_demo=first_demo, trial_num=seed)
        lfd_net.eval()
        new_file_root = matrix_file + str(first_demo) + '_' + str(seed) + '/'
        os.mkdir(os.path.join(new_file_root))
        fig_path_tmp = [ele + '/' for ele in new_file_root.split('/')]
        ...
        if koopmanoption == "Drafted":
            ...
            if not Unseen_test:
                errors = NGF_error_visualization(env_name, lfd_net, Training_data, Velocity, num_hand, num_obj, koopmanoption, error_type, False)
                if error_type == 'demo':
                ...
                if controller:
                    errors_simu = NGF_policy_control(env_name, Controller, lfd_net, Training_data, Velocity, num_hand, num_obj, koopmanoption, error_type, Visualize)
                    if error_type == 'demo':
                ...
                if not controller or error_type == 'demo':
                    ...
                    obj_ori_error_simu, obj_ori_error_simu_koopman, computed_joints_unseen, success_rate = NGF_policy_control_unseenTest(env_name, Controller, lfd_net, Training_data, Velocity, num_hand, num_obj, koopmanoption, Visualize)      
