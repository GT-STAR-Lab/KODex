...
def main(env_name, demo_file, num_demo, koopmanoption, velocity, save_matrix, matrix_file, config, control, error_type, visualize, unseen_test, rl_policy, folder_name, matrix_list, separate, seed, first_demo):
    ...
    Controller_loc = '/home/yhan389/Desktop/KoopmanManipulation_DataSplit/demo_data/Relocation_selfPlay/demo_controller_noisy_heavy_hand/NN_controller_best.pt'
    multiple_test = True
    if env_name is "":
        print("Unknown env.")
        return
    ...
    if len(folder_name) == 0: # we do not explicitly define the folder name
        folder_name = '/Relocate/NDP_cloning/parameter_tuning/'  # define the folder name for training
        unit_train = True
        ...
        if koopmanoption == "Drafted":
            ...
            if not Unseen_test:
                ...
                if not controller or error_type == 'demo':
                    ...
                    obj_ori_error_simu, obj_ori_error_simu_koopman, computed_joints_unseen, success_rate = NDP_policy_control_unseenTest(env_name, Controller, NDP_policy, Training_data, Velocity, num_hand, num_obj, koopmanoption, Visualize)      
